...
with open("duss_drilling_urls.json", "r") as f:
    urls = json.load(f)

temp_dir = "public/temp_duss_drilling"
os.makedirs(temp_dir, exist_ok=True)

raw_products = []
print(f"Starting to scrape {len(urls)} DUSS Drilling machines...")
...
for i, url in enumerate(urls):
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            # DUSS puts features inside <ul> elements inside a div with class "text"
            desc_en = ""
            features_en = []
            features_tr = []
            
            for text_div in soup.find_all('div', class_='text'):
                # Extract paragraph as description
                p = text_div.find('p')
                if p and len(p.get_text(strip=True)) > 20:
                    if not desc_en:
                        desc_en = p.get_text(" ", strip=True)
                
                # Extract UL as features
                ul = text_div.find('ul')
                if ul:
            ...
            raw_products.append({
                ...
                "solutions": ["profesyonel-delme-yikim"], # DUSS is mostly drilling and demolition
                "isFeatured": False
            })
...
with open("raw_duss_drilling.json", "w", encoding="utf-8") as f:
    json.dump(raw_products, f, ensure_ascii=False, indent=2)

print("Scraping and translation completed to raw_duss_drilling.json")
