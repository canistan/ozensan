      ...
      <section className="relative w-full h-[60vh] min-h-[500px] flex items-center bg-[#1A1E24] overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
          <div className="flex justify-between items-start mb-8">
            ...
            <Link href="/markalar" className="hidden sm:flex items-center text-sm font-bold text-white/70 hover:text-white transition-colors uppercase tracking-widest mt-1">
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
              {locale === "en" ? "BACK TO BRANDS" : "MARKALARA DÖN"}
            </Link>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 border-b border-neutral-200 pb-6">
            <div>
              ...
              <h2 className="text-3xl font-black text-[#1A1E24] tracking-tight">{t("allProducts")}</h2>
            </div>
              ...
              {brandProducts.map((prod) => (
                <Link href={`/urunler/${prod.slug}` as any} key={prod.slug} className="bg-white rounded-xl border border-neutral-200 overflow-hidden group hover:shadow-xl hover:border-[#C61A1A]/30 transition-all duration-300 flex flex-col">
                  ...
                  <div className="p-8 flex flex-col flex-grow bg-[#F8F9FA] border-t border-neutral-100 group-hover:bg-white transition-colors">
                    <h3 className="text-xl font-black text-[#1A1E24] group-hover:text-[#C61A1A] transition-colors mb-3 line-clamp-2">{locale === "en" && prod.nameEn ? prod.nameEn : prod.name}</h3>
                    <p className="text-sm text-[#8A95A5] line-clamp-2 mb-6">{locale === "en" && prod.descriptionEn ? prod.descriptionEn : prod.description}</p>
